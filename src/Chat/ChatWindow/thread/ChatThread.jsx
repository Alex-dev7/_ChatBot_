...
function ChatThread(props) {
    ...
    const [chatLog, setChatLog] = useState([
     { 
      user: "gpt",
      message: `Hello ${ userName ?  userName : "there"}! I'm Green Guru, your personal sustainability assistant.`
    },
    { 
      user: "user",
      message: ""
...
// fetch data on submit
    async function handleSubmit(event) {
        ...
        setChatLog([...chatLog,{user: "user", message: `${userPrompt}`}, {user: "gpt", message: data.messages[0][0].text.value}])
    ...
    <div style={{ ...threadStyles.chatThread }}>
            ...
            >
              {chatLog.map((message, index) => (
                  message.message === "" ? null : <ChatMessage key={index} message={message} />
              )
              )}
